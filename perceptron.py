...
class MultiPerceptron(object):
    ...
    """
    def __init__(self, num_classifiers, num_features, lr, num_epochs=20):
...
def main():
    ...
    # read/load data
    path_to_data_dir = '/Users/vanhoang/PycharmProjects/CL_Lab/data'
    data_file = 'NER_VLSP_2016.obj'

    if os.path.exists(os.path.join(path_to_data_dir, data_file)):
        print("Loading data files.....")
        with open(os.path.join(path_to_data_dir, data_file), 'rb') as file:
            data = pickle.load(file)
    else:
        print("Creating data file....")
        data = dataset.Dataset(path=path_to_data_dir)

        # save data as object for next time
        file = open(os.path.join(path_to_data_dir, data_file), 'wb')
    ...
    multi_classifier = MultiPerceptron(num_classifiers=len(data.label2idx), num_features=len(data.feature2idx), lr=0.1)
